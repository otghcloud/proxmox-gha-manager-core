...
<div data-node-form data-storage-url="{{ route('nodes.storage-options') }}" data-is-update="{{ $isUpdate ? 'true' : 'false' }}" data-target-id="{{ $target->id ?? '' }}">
	<div class="card">
		...
		<div class="card-body"><div class="row g-3">
			...
			<div class="col-md-6"><label class="form-label required" for="proxmox_token_id">API token ID</label><input class="form-control" id="proxmox_token_id" name="proxmox_token_id" required type="text" value="{{ old('proxmox_token_id', $target->proxmox_token_id) }}"></div>
			<div class="col-md-6"><label class="form-label {{ $isUpdate ? '' : 'required' }}" for="proxmox_token_secret">API token secret</label><input autocomplete="new-password" class="form-control" id="proxmox_token_secret" name="proxmox_token_secret" type="password" value="{{ old('proxmox_token_secret', $target->proxmox_token_secret) }}" {{ $isUpdate ? '' : 'required' }}></div>
